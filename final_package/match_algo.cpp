...
Matching max_matching(ED::Graph g, const std::string & filename){
    ...
    //much faster if perfct matching exists
    if(g.num_nodes()%2==0) {
    ...
    while (lowerBound < upperBound){
        ...
        if((k+(int)g.num_nodes()) % 2 == 1) k--; //otherwise there would not be a perf matching

        //build graph with k completely added vertices
        //start with g
    ...
    //find matching for best case (r added nodes), have to build it new
    //start with g
    ED::Graph graph = ED::Graph::build_graph(filename);
    ...
    //have to look at matching edges that do not cover the added nodes
    Matching M_best(g.num_nodes());

   //only add edges not covering the added nodes
   for(auto e: M._edges)
   {
        if(e._x < g.num_nodes() and e._y < g.num_nodes()) M_best.add_edge(e);
   }
