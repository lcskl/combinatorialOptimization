...
int main(int argc, char** argv)
{
   ...
   for(size_t i = 0; i < graph.num_edges(); ++i ) initialWeights[i] = graph.edge_weight(i);

   std::vector<NodeId> cycle = minimum_mean_weight_cycle(graph);

   output_file << "p edge " << graph.num_nodes() << " " << cycle.size() << std::endl;
   for(size_t i = 0; i < cycle.size(); ++i){
       output_file << "e " << graph.halfedge(2*cycle[i]).target()+1 << " " << graph.halfedge(2*cycle[i]+1).target()+1 << " " << initialWeights[cycle[i]] << std::endl;
   }
}
