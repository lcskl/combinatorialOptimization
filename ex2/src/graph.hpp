 ...
 * since we only declare the functions which read write our graph
 * from an std::istream or to an std::ostream, so we only include the forward declaration.
 */
#include <iosfwd>

/**
 * This header defines many different integer types,
 ...
 * This namespace is intended to be used for Minimum Mean Cycle.
 * */
namespace MMC
{
 ...
 * For example,  you can use signed integers here instead
 * if you want to avoid having to type cast when using the perfect matching solver.
 */
 ...
 * Note that the same Id may be used by different graphs though!
 */
...
// The inline keyword recommends the compiler to inline a function.
// If you use it for some function, you must implement that function
